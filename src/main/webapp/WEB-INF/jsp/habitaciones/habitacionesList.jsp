        ...
        <c:forEach items="${habitaciones}" var="hab">
            <tr>
               <td>
             	...
             	<td> <spring:url value="/reservas/{reservaId}/allHabitacionesDisponibles/{habitacionId}" var="reservahabUrl">
	                        <spring:param name="reservaId" value="${reservaId}"/> 
	                        <spring:param name="habitacionId" value="${hab.id}"/> 
	                </spring:url>
   					<a href="${fn:escapeXml(reservahabUrl)}" class="btn btn-default">Book room</a> </td>
            </tr>
        </c:forEach>
